-- Deployment-Ready ProjectPlanner Integration for ConstructBMS
-- Based on actual asta_tasks table structure
...
-- Using safe column names that are unlikely to conflict
...
CREATE INDEX IF NOT EXISTS idx_asta_projects_planner_budget ON asta_projects(projectplanner_budget);
CREATE INDEX IF NOT EXISTS idx_asta_projects_planner_cost ON asta_projects(projectplanner_actual_cost);
...
-- RLS policies using the actual assigned_to column from asta_tasks
CREATE POLICY "Users can view projectplanner data for assigned projects" ON projectplanner_milestones
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY "Users can view projectplanner resources for assigned projects" ON projectplanner_resources
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM asta_tasks
...
CREATE POLICY "Users can view projectplanner documents for assigned projects" ON projectplanner_documents
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM asta_tasks
...
CREATE POLICY "Users can view projectplanner comments for assigned projects" ON projectplanner_comments
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM asta_tasks
...
-- Function to update project budget when tasks change
CREATE OR REPLACE FUNCTION update_project_budget()
RETURNS TRIGGER AS $$
BEGIN
    -- Update the project's budget when a task cost changes
    UPDATE asta_projects
    SET projectplanner_budget = (
...
-- Trigger to automatically update project budget when task costs change
CREATE TRIGGER update_project_budget_trigger
    AFTER UPDATE OF projectplanner_cost ON asta_tasks
    FOR EACH ROW
...
SELECT
    ...
    CASE
        ...
        ELSE (COUNT(t.id) FILTER (WHERE t.progress = 100) * 100) / COUNT(t.id)
...
SELECT
    ...
    p.projectplanner_budget as project_budget,
