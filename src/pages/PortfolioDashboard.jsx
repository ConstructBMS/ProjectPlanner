...
import ProjectPlannerFeatures from '../modules/ProgrammeManager/components/ProjectPlannerFeatures';
import {
  ChartBarIcon,
  ClockIcon,
  CurrencyDollarIcon,
...
const PortfolioDashboard = ({ onProjectSelect, onShowTemplates }) => {
  ...
  // Debug logging
  console.log('PortfolioDashboard context state:', {
    projects: projects?.length,
    loading,
    error,
    getProjectsByStatus: typeof getProjectsByStatus,
  ...
  const statusChartData = useMemo(() => {
    ...
    return statuses.map(status => ({
      ...
      count: (() => {
        try {
          return getProjectsByStatus && projects
            ? getProjectsByStatus(status).length
            : 0;
        } catch (error) {
          console.warn('Error calculating status count:', error);
          return 0;
        }
      })(),
  ...
  const progressChartData = useMemo(() => {
    ...
    return ranges.map(range => ({
      ...
      count: projects.filter(
        p => p.progress >= range.min && p.progress <= range.max
      ).length,
    }));
  }, [projects]);
  ...
  // Safety check for context functions and metrics
  if (!getProjectsByStatus || !projects || !metrics) {
    console.log(
      'PortfolioDashboard: Context not ready, showing loading state',
      {
    ...
    return (
      <div className='min-h-screen bg-gray-50 flex items-center justify-center'>
        <div className='text-center'>
          <div className='animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4' />
          <p className='text-gray-600'>Initializing Portfolio Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
                  ...
                  <div className='flex-1'>
                    ...
                    <div className='mt-1 w-full bg-gray-200 rounded-full h-2'>
                      <div
                        ...
                        style={{
                          width: `${(item.count / (metrics?.totalProjects || 1)) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        {/* ProjectPlanner Enhanced Features */}
        <div className='mb-8'>
          <ProjectPlannerFeatures />
        </div>
                          ...
                          <div className='flex items-center text-sm text-gray-500 mt-1'>
                            <span
                              className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                ...
                                project.projectplanner_priority === 'critical'
                                  ...
                                  : project.projectplanner_priority === 'medium'
                            ...
                            >
                              {project.projectplanner_priority
                                .charAt(0)
                                .toUpperCase() +
                                project.projectplanner_priority.slice(1)}{' '}
                              Priority
                            </span>
                          </div>
                        )}
                        ...
                        <div>
                          {project.projectplanner_estimated_hours && (
                            <div className='flex items-center text-sm text-gray-600'>
                              ...
                              Est:{' '}
                              {project.projectplanner_estimated_hours.toLocaleString()}
                              h
                            </div>
                          )}
                          {project.projectplanner_actual_hours && (
                            <div className='flex items-center text-sm text-gray-600'>
                              ...
                              Actual:{' '}
                              {project.projectplanner_actual_hours.toLocaleString()}
                              h
                            </div>
                          )}
                        </div>
