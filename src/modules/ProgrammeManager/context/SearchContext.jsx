...
export const SearchProvider = ({ children }) => {
  ...
  const mockResources = useMemo(() => [
    { id: 'resource-1', name: 'Crane Operator', type: 'Equipment', capacity: 100 },
    { id: 'resource-2', name: 'Concrete Worker', type: 'Labor', capacity: 150 },
    ...
    { id: 'resource-6', name: 'Project Manager', type: 'Management', capacity: 100 },
    { id: 'resource-7', name: 'Site Engineer', type: 'Engineering', capacity: 100 },
    { id: 'resource-8', name: 'Safety Officer', type: 'Safety', capacity: 100 },
  ], []);
    ...
    try {
      ...
      const taskResults = tasks
        .filter(task => {
          ...
          const descriptionMatch = task.notes?.toLowerCase().includes(searchTerm);
          ...
          return nameMatch || descriptionMatch || assigneeMatch || statusMatch;
        })
        .map(task => ({
          id: task.id,
          type: 'task',
      ...
      const projectResults = projects
        .filter(project => {
          ...
          const managerMatch = project.manager?.toLowerCase().includes(searchTerm);
          const locationMatch = project.location?.toLowerCase().includes(searchTerm);
          const statusMatch = project.status?.toLowerCase().includes(searchTerm);
          
          return nameMatch || descriptionMatch || managerMatch || locationMatch || statusMatch;
        })
        .map(project => ({
          id: project.id,
          type: 'project',
      ...
      const allResults = [...taskResults, ...resourceResults, ...projectResults];
