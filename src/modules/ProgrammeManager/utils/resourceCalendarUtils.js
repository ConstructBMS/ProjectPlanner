 ...
 */
export const getResourceCalendarId = (resource, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  ...
  if (config.fallbackToProjectCalendar && projectCalendars[config.defaultCalendarId]) {
 ...
 */
export const getResourceCalendar = (resource, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
 ...
 */
export const isResourceAvailableOnDate = (resource, date, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
 ...
 */
export const getResourceWorkingHours = (resource, date, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const calendar = getResourceCalendar(resource, projectCalendars, config);
 ...
 */
export const calculateResourceAvailability = (resource, startDate, endDate, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const calendar = getResourceCalendar(resource, projectCalendars, config);
  ...
  while (currentDate <= endDate) {
    const workingHours = getResourceWorkingHours(resource, currentDate, projectCalendars, config);
 ...
 */
export const checkResourceAllocationConflicts = (resource, startDate, endDate, existingTasks, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const conflicts = [];
  const calendar = getResourceCalendar(resource, projectCalendars, config);
  ...
  if (!calendar) {
    return {
      hasConflicts: true,
      conflicts: [{ type: 'no_calendar', message: 'Resource has no assigned calendar' }],
      totalConflicts: 1,
    };
  }

  // Check if task dates fall within resource's working days
  const availability = calculateResourceAvailability(resource, startDate, endDate, projectCalendars, config);
  ...
  if (config.respectResourceAvailability) {
    const weeklyAllocation = calculateWeeklyAllocation(resource, startDate, endDate, existingTasks, projectCalendars, config);
 ...
 */
export const calculateWeeklyAllocation = (resource, startDate, endDate, existingTasks, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const weeklyHours = {};
  const calendar = getResourceCalendar(resource, projectCalendars, config);
  ...
  existingTasks.forEach(task => {
    ...
    while (taskCurrentDate <= taskEnd) {
      ...
      if (weeklyHours[weekKey] !== undefined) {
        const workingHours = getResourceWorkingHours(resource, taskCurrentDate, projectCalendars, config);
        weeklyHours[weekKey] += workingHours.totalHours;
      }
  ...
  const averageWeeklyHours = weeklyHoursArray.length > 0 
    ? weeklyHoursArray.reduce((sum, hours) => sum + hours, 0) / weeklyHoursArray.length 
    : 0;
...
export const validateResourceCalendar = (resource, projectCalendars) => {
  ...
  if (resource.calendarId && !projectCalendars[resource.calendarId]) {
    errors.push(`Calendar "${resource.calendarId}" not found in project calendars`);
  }
 ...
 */
export const getAvailableCalendars = (projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const calendars = [];
 ...
 */
export const updateResourceCalendar = (resource, calendarId, projectCalendars) => {
 ...
 */
export const getResourceCalendarSummary = (resource, projectCalendars, config = DEFAULT_RESOURCE_CALENDAR_CONFIG) => {
  const calendar = getResourceCalendar(resource, projectCalendars, config);
  ...
  const workingDays = Object.values(calendar.workingDays).filter(day => day).length;
