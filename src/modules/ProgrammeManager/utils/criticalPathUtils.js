...
export const calculateCriticalPath = (tasks, taskLinks, getCalendarForTask) => {
  ...
  const tasksWithDates = performForwardBackwardPass(
    tasks,
    taskLinks,
    getCalendarForTask
  );
...
const performForwardBackwardPass = (tasks, taskLinks, getCalendarForTask) => {
  ...
  const forwardPass = performForwardPass(
    updatedTasks,
    taskLinks,
    getCalendarForTask
  );
  ...
  const backwardPass = performBackwardPass(
    forwardPass,
    taskLinks,
    getCalendarForTask
  );
...
const performForwardPass = (tasks, taskLinks, getCalendarForTask) => {
  ...
  startTasks.forEach(task => {
    ...
    if (taskIndex !== -1) {
      updatedTasks[taskIndex] = {
        ...
        earliestFinish: addDays(
          new Date(task.startDate),
          task.duration - 1,
          getCalendarForTask(task.id, task)
        ),
      };
      visited.add(task.id);
    }
  ...
  while (changed) {
    ...
    updatedTasks.forEach((task, index) => {
      ...
      const predecessors = taskLinks.filter(link => link.toTaskId === task.id);
      const unvisitedPredecessors = predecessors.filter(
        link => !visited.has(link.fromTaskId)
      );
  ...
  while (changed) {
    ...
    updatedTasks.forEach((task, index) => {
      ...
      const unvisitedSuccessors = successors.filter(
        link => !visited.has(link.toTaskId)
      );
      ...
      if (unvisitedSuccessors.length === 0) {
        ...
        // Calculate float
        const totalFloat = Math.ceil(
          (latestFinish.getTime() - task.earliestFinish.getTime()) /
            (1000 * 60 * 60 * 24)
        );
...
const calculateFreeFloat = (task, successors, allTasks, getCalendarForTask) => {
  ...
  const freeFloat = Math.ceil(
    (minSuccessorStart.getTime() - task.earliestFinish.getTime()) /
      (1000 * 60 * 60 * 24)
  );
...
export const getCriticalPathStats = tasks => {
  ...
  const criticalPercentage =
    totalTasks > 0 ? Math.round((criticalCount / totalTasks) * 100) : 0;

  // Calculate critical path duration
  let criticalPathDuration = 0;
  if (criticalTasks.length > 0) {
    const earliestStart = new Date(
      Math.min(...criticalTasks.map(t => new Date(t.startDate)))
    );
    const latestFinish = new Date(
      Math.max(...criticalTasks.map(t => new Date(t.endDate)))
    );
    criticalPathDuration =
      Math.ceil(
        (latestFinish.getTime() - earliestStart.getTime()) /
          (1000 * 60 * 60 * 24)
      ) + 1;
  }
