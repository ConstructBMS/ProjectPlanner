...
const ColumnChooserDialog = ({
  ...
  const isColumnInGrid = useCallback((columnKey) => {
  ...
  const handleToggleVisibility = useCallback((columnKey) => {
    if (!onConfigChange) return;
  ...
  const handleDrop = useCallback((e, targetColumnKey) => {
    ...
    if (!dragState.isDragging || !dragState.draggedItem || !onConfigChange) return;
    
    const draggedKey = dragState.draggedItem;
    const targetKey = targetColumnKey;
    
    if (draggedKey === targetKey) {
      setDragState({ isDragging: false, draggedItem: null, draggedOverItem: null });
      return;
    }
    
    // Find indices
    const draggedIndex = currentColumns.findIndex(col => col.key === draggedKey);
    const targetIndex = currentColumns.findIndex(col => col.key === targetKey);
                    ...
                    >
                      <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {column.category && (
                              <span className={`px-2 py-1 rounded-full text-xs font-medium bg-${
                                COLUMN_CATEGORIES[column.category]?.color || 'gray'
                              }-100 text-${
                                COLUMN_CATEGORIES[column.category]?.color || 'gray'
                              }-800`}>
                        ...
                        <button
                          ...
                          title={isColumnInGrid(column.key) ? 'Already added' : 'Add column'}
                        >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <span className="font-medium text-gray-900">
                                  {column.label || COLUMN_TYPES[column.key]?.label}
                                </span>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              ...
              {currentColumns.filter(col => col.visible).length !== currentColumns.length && (
